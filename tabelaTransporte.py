...
def alterar_produto(conexao):
    ...
    sql = """ SELECT rowid, * FROM transporte;
    """
    cursor.execute(sql)
    ...
    rowid = int(input("Digite o ID do transporte a ser alterado: "))
...
def deleteProduto(conexao):
    ...
    if(delete == 'Sim' or delete == 's' or delete == 'S'):
        sql_excluir = '''
            DELETE FROM transporte WHERE rowid = {}
        '''.format(id)
...
def relatorio(conexao):
    ...
    sql = '''
        ...
        FROM transporte INNER JOIN cliente ON transporte.idcli = cliente.rowid
    '''
